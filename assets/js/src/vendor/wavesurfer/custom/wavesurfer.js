 ...
 *
 * @param {?boolean} loop True to loop the audio playback (e.g. during async events)
 * @return {object|null} temporary elements for async cleanup
 *
 * @since 5.2.0
 */
function ignoreSilenceMode() {
  ...
  var tmpEls = null; // disable iOS Airplay (setting the attribute in js doesn't work)

  var tmp = document.createElement("div");
  tmp.innerHTML = '<audio x-webkit-airplay="deny"></audio>';
  var audioSilentMode = tmp.children.item(0);
  audioSilentMode.loop = loop;
  audioSilentMode.preload = "auto";
  audioSilentMode.type = "audio/mpeg";
  audioSilentMode.disableRemotePlayback = true;
  audioSilentMode.addEventListener('loadstart', handleLoadStart, {
    capture: true,
    once: true
  });
  audioSilentMode.src = audioData;
  /**
   * Handles `loadstart` event. Begins playback and cleans up temporary elements.
   *
   * Google Chrome play() fails and throws error `DOMException: The play() request
   * was interrupted...` if not called from event listener
   */

  function handleLoadStart() {
    // play
    var promise = audioSilentMode.play(); // cleanup temporary elements for synchronous calls

    if (promise !== undefined) {
      promise.catch(function (error) {// Ignore `NotAllowedError` thrown when play() is called asynchronously
      }).finally(function () {
        if (!loop) {
          audioSilentMode.removeEventListener('loadstart', handleLoadStart);
          audioSilentMode.remove();
          tmp.remove();
          audioSilentMode = null;
          tmp = null;
        }
      });
    }
  ...
  if (loop) {
    tmpEls = {
      tmpEl: tmp,
      audioEl: audioSilentMode,
      handle: handleLoadStart
    };
  } // return temporary elements for async cleanup
...
var WaveSurfer = /*#__PURE__*/function (_util$Observer) {
  ...
  _createClass(WaveSurfer, [{
     ...
     * Async param allows bypassing of ignoreSilenceMode().
     *
     * @param {?number} start Position to start at
     * @param {?number} end Position to end at
     * @param {?boolean} async True to bypass ignoreSilenceMode()
    ...
    value: function play(start, end) {
      ...
      if (this.params.ignoreSilenceMode && !async) {
        // ignores device hardware silence mode
        util.ignoreSilenceMode();
      }

      this.fireEvent('interaction', function () {
        return _this7.play(start, end, true);
      });
